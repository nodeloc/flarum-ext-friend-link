...
export default class ImageCropModal extends Modal {
      ...
      <div className="Modal-body">
        ...
        {this.ready && this.cropper && (
          <p className="helpText">
            {app.translator.trans('nodeloc-friend-link.forum.modal.help_text', {
  ...
  async loadPicker(evt) {
    ...
    if (!Cropper) {
      this.alertAttrs = {
        type: 'error',
        content: app.translator.trans('nodeloc-friend-link.forum.modal.error.failed_to_load_cropper'),
      };
    }
  ...
  async upload() {
    ...
    try {
      ...
      console.error('[nodeloc/friend-link] An error occurred while resizing the image.', e);

      this.alertAttrs = {
        type: 'error',
        content:
          e.code === 'ERR_GET_IMAGE_DATA'
            ? app.translator.trans('nodeloc-friend-link.forum.modal.error.get_image_data')
            : app.translator.trans('nodeloc-friend-link.forum.modal.error.generic_resize'),
      };
